...
function Journal() {
  ...
  const handlePasswordSubmit = (password) => {
    if (modalConfig.action === 'save') {
      // 1. Setup Mode: Save the password for the session, close modal, and execute save
      setSessionPassword(password);
      setModalConfig({ show: false, mode: '', action: null, payload: null });
      executeSave(password);
      
    } else if (modalConfig.action === 'load') {
      // 2. Unlock Mode: Try to decrypt the payload
      try {
        ...
        setSessionPassword(password); // Remember it so they can easily save edits!
        setModalConfig({ show: false, mode: '', action: null, payload: null });
      } catch (error) {
  ...
  // ==========================================
  const handleExit = () => {
    // 1. Instantly wipe the encryption key from active memory
    setSessionPassword('');
  ...
  const triggerLoad = async () => {
    ...
    if (window.electronAPI) {
      loadedContent = await window.electronAPI.loadJournal();
    } else {
      loadedContent = localStorage.getItem(fileName);
    }

    if (loadedContent) {
      // Check for the CryptoJS signature to see if it's encrypted
      if (loadedContent.startsWith('U2FsdGVkX1')) {
         setModalConfig({ show: true, mode: 'decrypt', action: 'load', payload: loadedContent });
      } else {
         setEntry(loadedContent); // Normal plain text
      }
    }
  };
  ...
  const triggerSave = async () => {
    ...
    // If encryption is ON, and we don't have a password yet -> Ask for it!
    if (settings.enableEncryption && !sessionPassword) {
      setModalConfig({ show: true, mode: 'encrypt', action: 'save', payload: null });
    } else {
      // Otherwise, just save normally (using session password if it exists)
  ...
  const executeSave = async (passwordToUse) => {
    ...
    // Apply AES-256 Encryption if requested
        ...
        <div className="mac-desktop">
          ...
          <div className="mac-window">
             <div className="mac-title-bar">
                {/* FIXED: Replaced window.history.back() with handleExit */}
          ...
          {retro === 'e-ink' ? (
            ...
            <div className="retro-status-bar">
               ...
               <div className="command-group">
                 <button onClick={triggerSave}>{retro === 'commodore-64' ? '[F1] SAVE' : '[CTRL-S] SAVE'}</button>
                 {/* FIXED: Replaced window.history.back() with handleExit */}

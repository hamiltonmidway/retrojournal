...
function Home() {
  ...
  // Default to the first vibe selected
      ...
      <div className="gallery-section">
         ...
         <div className="vibe-grid">
            {vibes.map(vibe => (
               <div
                 key={vibe.id}
                 // If this card is selected, add the 'active-card' class (we'll style this to glow)
                 className={`vibe-card ${vibe.id} ${selectedVibe.id === vibe.id ? 'active-card' : ''}`}
                 onClick={() => setSelectedVibe(vibe)}
               >
                 {/* === PASTE YOUR BIG LOGO LOGIC HERE === */}
